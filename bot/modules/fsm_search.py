...
async def fsm_callback(client, callback_query) :
    ...
    try :
        ...
        # 处理优惠类型选择
            ...
            await edit_message(message, f"<b>🔍 正在搜索:</b> <i>{keyword}</i>...", disable_web_page_preview=True)
        ...
        # 处理下载按钮
            ...
            try :
                ...
                if not torrent_details.get('success', False) :
                    await edit_message(message, f"<b>❌ 获取种子详情失败:</b> {torrent_details.get('msg', '未知错误')}")
                    return
            ...
            try :
                ...
                if not download_url :
                    await edit_message(message, "<b>❌ 无法获取下载链接</b>")
                    return
            except Exception as e :
                LOGGER.error(f"获取下载链接错误: {e}")
                await edit_message(message, f"<b>❌ 获取下载链接失败:</b> {str(e)}")
                return
        ...
        # 检查是否是MESSAGE_NOT_MODIFIED错误，如果是则忽略
...
async def handle_search_results(client, message, search_results, user_id) :
    ...
    try :
        ...
        # 设置disable_web_page_preview=True防止URL预览
        await edit_message(message, result_msg, button, disable_web_page_preview=True)
